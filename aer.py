...
class AERSufficientStatistics:
    ...
    def __init__(self, gold_alignments, predicted_alignments):
        self.a_and_s = 0
        self.a_and_p = 0
        self.a = 0
        self.s = 0
        ...
        for i in range(len(gold_alignments)):
            self._update(gold_alignments[i][0], gold_alignments[i][1], predicted_alignments[i])

    def __str__(self):
        return '%s/%s/%s/%s %s' % (self.a_and_s, self.a_and_p, self.a, self.s, self.aer())

    def _update(self, sure, probable, predicted):
        """
...
def test(path_gold, path_predicted):

    from random import random
    # 1. Read in gold alignments
    gold_sets = read_naacl_alignments(path_gold)
    ...
    predictions_raw = read_naacl_alignments(path_predicted)
    ...
    for gold, pred in zip(gold_sets, predictions):
        metric._update(sure=gold[0], probable=gold[1], predicted=pred)
    # AER
    print(metric.aer())
...
if __name__ == '__main__':
    ...
    data_pickle = 'pickles/data-t1.pickle'
    ...
    #
    # model.write_NAACL_alignments('test_data')
    #
    # test('data/testing/answers/test.wa.nonullalign', 'alignments/test_data.txt')
